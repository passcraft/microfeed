    ...
    return (
      <div className="lh-page-card">
        <div className="lh-page-title">
          What's new from{" "}
          <a href={`https://${OUR_BRAND.domain}`}>{OUR_BRAND.domain}</a>?
        </div>
        <div>
          {fetching ? (
            <div className="text-muted-color text-sm">Loading...</div>
          ) : (
              ...
              {items.map((item) => (
                <div key={`item-${item.id}`}>
                  ...
                  <div className="text-xs text-muted-color mt-1">
                    {item._yaar.date_published_short}
                  </div>
                </div>
              ))}
              {items.length > 0 ? (
                <div className="text-right">
                  <a href={OUR_BRAND.whatsnewWebsite} target="_blank">
                    Read more <span className="lh-icon-arrow-right" />
                  </a>
                </div>
              ) : (
                <div className="-text-xs text-muted-color mt-1">No news.</div>
              )}
            </div>
          )}
        </div>
