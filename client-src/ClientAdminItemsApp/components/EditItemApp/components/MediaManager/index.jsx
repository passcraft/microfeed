...
function PreviewCurrentMediaFile({
  url,
  contentType,
  category,
  durationSecond,
    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-1 text-sm">
          <div className="mb-1">
            <span className="text-helper-color">Content type:</span>{" "}
            {contentType}
          </div>
          <div className="mb-1">
            <span className="text-helper-color">File size:</span>{" "}
            {humanFileSize(sizeByte)}
          </div>
          {[ENCLOSURE_CATEGORIES.AUDIO, ENCLOSURE_CATEGORIES.VIDEO].includes(
            category
          ) && (
            <div className="mb-1">
              <span className="text-helper-color">Duration:</span>{" "}
              {secondsToHHMMSS(durationSecond)}
            </div>
          )}
          <div className="break-all">
            <span className="text-helper-color">Download url:</span>{" "}
            <a href={url} className="text-xs" target="_blank">
              {url}
            </a>
          </div>
        </div>
...
function MediaUploader({
  ...
  const headerTitle = fileNotExist
    ? `Upload a new ${category} file to replace this one`
    : `Upload a new ${category} file`;
  return (
    <div>
      {url && (
        <PreviewCurrentMediaFile
      ...
      <details className="lh-upload-wrapper" open={!fileNotExist}>
        <summary className="m-page-summary mt-4 text-sm">{headerTitle}</summary>
        <FileUploader
              ...
              <div className="text-brand-light">
                <div className="flex items-center">
                  ...
                  <div className="font-semibold">
                    Click or drag here to upload {category}
                  </div>
                </div>
...
function UrlEditor({ url, onUpdateUrl }) {
  const bookmarkletCode =
    `javascript:window.location=%22${ADMIN_URLS.newItem(
      getPublicBaseUrl()
    )}?media_category=external_url&` +
    "media_url=%22+encodeURIComponent(document.location)+%22&title=%22+encodeURIComponent(document.title)";
  const bookmarklet = `<a href="${bookmarkletCode}" onclick="return false" rel="nofollow">to yaar</a>`;
  return (
    <div>
      <AdminInput
      ...
      <details className="mt-4 text-helper-color">
        <summary className="hover:opacity-50 text-sm cursor-pointer">
          Bookmarklet: add a "to yaar" button to browser
        </summary>
        <div className="mt-4 text-sm">
          Drag this link to your browser, so you can easily curate web pages
          here -
          <div
...
export default class MediaManager extends React.Component {
  constructor(props) {
    ...
    let { url, category, contentType, sizeByte, durationSecond } =
      mediaFile || {};
  ...
  onFileUpload(file) {
    ...
    Requests.upload(
      ...
      () => {
        showToast(
          "Failed to upload. Please refresh this page and try again.",
          "error",
          2000
        );
  ...
  render() {
    ...
    return (
      <div>
        {label && <h2 className="lh-page-title">{label}</h2>}
        {labelComponent}
        <div className="flex">
          <AdminRadio
            groupName="category"
            ...
            onChange={(e) => {
              if (url) {
                ...
                const ok = confirm(
                  `To switch to ${newName}, you should discard ${name} first. This will delete existing ${name}. Do you want to proceed?`
                );
                if (!ok) {
                  return;
                }
