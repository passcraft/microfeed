...
export default class WebGlobalSettingsApp extends React.Component {
  constructor(props) {
    ...
    if (feed.settings && feed.settings[currentType]) {
      ...
      itemsSortOrder =
        feed.settings[currentType].itemsSortOrder ||
        ITEMS_SORT_ORDERS.NEWEST_FIRST;
      itemsPerPage =
        feed.settings[currentType].itemsPerPage || DEFAULT_ITEMS_PER_PAGE;
    }
    this.state = {
      feed,
    ...
    return (
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          <details open>
            <summary className="lh-page-subtitle cursor-pointer">
            ...
            <AdminInput
              ...
              onChange={(e) =>
                this.setState({ publicBucketUrl: e.target.value }, () =>
                  setChanged()
                )
              }
            />
              ...
              <div className="col-span-1">
                <AdminRadio
                  ...
                  labelComponent={
                    <ExplainText
                      bundle={
                        CONTROLS_TEXTS_DICT[SETTINGS_CONTROLS.ITEMS_SORT_ORDER]
                      }
                  ...
                  onChange={(e) =>
                    this.setState({ itemsSortOrder: e.target.value }, () =>
                      setChanged()
                    )
                  }
                />
              ...
              <AdminImageUploaderApp
                ...
                imageSizeNotOkayMsgFunc={(width, height) => {
                  if (width > 256 && height > 256) {
                    return (
                      `Image too big: ${parseInt(width)} x ${parseInt(
                        height
                      )} pixels. ` +
                    ...
                    return (
                      `Image too small: ${parseInt(width)} x ${parseInt(
                        height
                      )} pixels. ` +
                ...
                onImageUploaded={(cdnUrl, contentType) =>
                  this.setState(
                    {
                      favicon: {
                        url: cdnUrl,
