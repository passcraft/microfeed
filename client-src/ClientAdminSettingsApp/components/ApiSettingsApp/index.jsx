...
export default class ApiSettingsApp extends React.Component {
  ...
  setApiEnabled(checked) {
    this.setState(
      { apiBundle: { ...this.state.apiBundle, enabled: checked } },
      () => {
      ...
      >
        <div className="mb-4">
          <div className="">
            <AdminSwitch
              label="API Enabled"
              customLabelClass={clsx(
                "",
                apiBundle.enabled ? "text-black" : "text-muted-color"
              )}
              enabled={apiBundle.enabled}
              setEnabled={(checked) => this.setApiEnabled(checked)}
            />
            <div className="text-muted-color text-xs mt-2">
              You can use the API to manage contents of your feed, e.g., create,
          ...
          <div className="flex items-center mt-8">
            <div
              className={clsx(
                "flex-none text-sm",
                !apiBundle.enabled && "text-muted-color"
              )}
            >
              API Key:
            </div>
            <div className="flex-1 mx-2">
              <AdminInput
                ...
                value={app.token}
                customClass={clsx(
                  "text-sm p-1 select-all",
                  !apiBundle.enabled && "text-muted-color"
                )}
                description={
                  "Set the X-yaarAPI-Key header to the API key, e.g., " +
                  `curl -H X-yaarAPI-Key: ${app.token} ...`
                }
              />
            </div>
            <div className="flex-none">
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  const ok = confirm(
                    "Are you sure you want to reset the API key?"
                  );
                  if (ok) {
          ...
          <div className="text-xs mt-8">How to use API key?</div>
          <div className="mt-2 text-xs text-helper-color">
            {"Set the X-yaarAPI-Key header to the API key, e.g., " +
              'curl -H "X-yaarAPI-Key: <API_KEY>" ...'}
          </div>
          <div className="mt-8">
            <a
