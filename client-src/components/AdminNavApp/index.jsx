...
function NavItem({ url, title, navId, currentId, Icon, disabled }) {
  return (
    ...
    >
      <div
        className={clsx(
          "py-4 px-4 xl:px-8 flex items-center",
          navId === currentId
      ...
      >
        ...
        <div className="text-sm xl:text-xl">{title}</div>
      </div>
    </a>
  );
}
...
export default class AdminNavApp extends React.Component {
  ...
  render() {
    ...
    const onboardingResult = this.props.onboardingResult || {
      requiredOk: true,
    };
    return (
      <div className="flex flex-col min-h-screen min-w-screen">
        <div className="grid grid-cols-12 gap-4 bg-white flex items-center border-b drop-shadow">
          ...
          <div className="col-span-10 flex items-center">
            {upperLevel && (
              <div className="py-6 pl-4 xl:pl-16">
                <a href={upperLevel.url}>
                  <span className="lh-icon-arrow-left" /> {upperLevel.name}
                </a>
        ...
        <div className="grid grid-cols-12 gap-4 flex-1">
          ...
          <div className="col-span-10 w-full">
            <div className="py-8 px-4 xl:px-16">{this.props.children}</div>
          </div>
        </div>
