...
/**
 * How to upload a file to R2 via yaar API:
 * 1) Get presigned url via
 *    curl -X POST $yaar_url/api/media_files/presigned_urls/ \
...
export async function onRequestPost(context) {
  ...
  if (!jsonInputParams) {
    status = 400;
    error =
      "You have to provide json input params: " +
    ...
    if (!["image", "audio", "video", "document"].includes(category)) {
      ...
      status = 400;
      error =
        "You have to provide item_id for non-image categories, e.g., " +
    ...
    const extension = full_local_file_path.slice(
      ((full_local_file_path.lastIndexOf(".") - 1) >>> 0) + 2
    );
  ...
  const resultJson = {
    ...
    media_url: urlJoinWithRelative(
      publicBucketUrl,
      `${jsonData.mediaBaseUrl}/${r2ObjectKey}`
    ),
  };
